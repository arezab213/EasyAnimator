...
public class AnimationReader {
   ...
   * <li>Slow-mo lines: the keyword "slow-mo" followed by a start time, end time and
   * a desired tempo in ticks per second to specify periods during the animation when
   * certain playback speed is desired.</li>
   * </ul>
   * </p>
   *
  ...
  public static <Doc> Doc parseFile(Readable readable, AnimationBuilder<Doc> builder) {
    ...
    Objects
        .requireNonNull(builder, "Must provide a non-null cs3500.animator.model.AnimationBuilder");
  ...
  private static <Doc> void readSlowMo(Scanner s, AnimationBuilder<Doc> builder) {
    int startTick;
    int endTick;
    int speed;
    if (s.hasNextInt()) {
      ...
      throw new
          IllegalStateException("Slow-mo: Expected a start time, but no more input available");
    }
    if (s.hasNextInt()) {
      endTick = s.nextInt();
    } else {
      throw new IllegalStateException("Slow-mo: Expected an end time, but no more input available");
    }
    if (s.hasNextInt()) {
      speed = s.nextInt();
    } else {
      throw new IllegalStateException("Slow-mo: Expected a speed, but no more input available");
    }
    builder.declareSlowMo(startTick, endTick, speed);
  }
